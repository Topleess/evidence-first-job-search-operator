...
def main()->int:
 p=argparse.ArgumentParser();p.add_argument('--db',default='state/job_funnel.sqlite3');p.add_argument('--limit',type=int,default=5);p.add_argument('--execute',action='store_true');p.add_argument('--profile',default='/opt/data/job-search-agent-linkedin/data/browser_profiles/linkedin');p.add_argument('--resume',default='resume/resume_product_manager_alexander_shamshurin_2026-07-09.pdf');p.add_argument('--evidence-dir',default='state/linkedin-evidence');p.add_argument('--executor',default=str(Path(__file__).with_name('linkedin_easy_apply_executor.js')));a=p.parse_args()
 ...
 try:
  for c in candidates:
   cmd=['node',str(a.executor),'--job-id',c.external_id,'--run-id',run,'--search-url',c.search_url,'--db',str(a.db),'--profile',str(a.profile),'--resume',str(a.resume),'--evidence-dir',str(a.evidence_dir)]
   if not a.execute: cmd.append('--dry-run')
   ...
   if proc.returncode != 0:
    terminal_state='failed';reason=f'executor_failed job_id={c.external_id} exit_code={proc.returncode}'
    break
  if terminal_state=='completed': reason=f"candidates={len(candidates)} execute={a.execute}"
